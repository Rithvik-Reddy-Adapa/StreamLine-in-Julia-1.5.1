...
xi=collect(0.1:0.1:1.1)
yi=ones(size(xi))
zi=zeros(size(xi))
xo,yo,zo,mag=StreamLine(x=x,y=y,z=z,u=u,v=v,w=w,xi=xi,yi=yi,zi=zi,n=100,length=0.01,r_mag=true)

p1=Plot([ scatter3d(x=xo[i,:], y=yo[i,:], z=zo[i,:], mode="lines", line=attr(width=2,colorscale="Viridis", showscale=true ), line_color=range(0,3,length=100), showlegend=false ) for i=1:11 ], Layout(title="Stream Line"))
display(p1)
plot(p1)
#savehtml(p1,raw"Path\to\file\Example1.html")
